function[tod,dx,dy]=read_tod_header_abs_corrected(todname,row,col,ra_wrap,varargin)
...
try
  ct=getdata_double_channel(myf,'corr_sync_time');
catch
...
if numel(sync_box_num)~=numel(az)
  ...
  try
    sync_num=getdata_double_channel(myf,'corr_sync_number');
  catch
...
if (do_reconstruct_hwp)
  ...
  try
    hwp=getdata_double_channel(myf,'corr_hwp_encoder_counts');
  catch
